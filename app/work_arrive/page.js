...
const handleLocationClick = async (location) => {
    ...
    const response = await fetch('https://gummy-dang.com', {
        ...
        body: JSON.stringify({ endLocation: location.place_name }),
    });
    ...
    router.push({
        pathname: "/work_input",
        query: { endLocation: location.place_name },
    });
    } catch (error) {
    console.error('도착 위치 전송 중 문제가 발생했습니다:', error);
    alert('도착 위치 전송 중 오류가 발생했습니다. 다시 시도해 주세요.');
    }
};
...
return (
    ...
    <div className={styles.header}>
        ...
        <h1 className={styles.title}>도착지</h1>
    </div>
